...
export class DoctorCollectionPresenter extends CollectionPresenter {
    data: DoctorPresenter[];

    constructor(output: { data: Doctor[]; paginationProps: PaginationPresenterProps }) {
        const { data, paginationProps } = output;
        super(paginationProps);
        this.data = data.map((i: Doctor) => new DoctorPresenter(i));
    }
}
